...
has 5 matching numbers, it would then win a copy of the same cards that the original 
card 10 won: cards 11, 12, 13, 14, and 15. This process repeats until none of the copies 
...
with open('d:\\python\\AOC_input_4.txt', 'r') as file:
...
def raw_to_map(raw_game_list, mapped_input):
    for string_with_game in raw_game_list:
        ...
        ticket_numbers = [int(i) for i in string_with_game[string_with_game.find(':') + 1 : string_with_game.find('|')].split()]
        user_numbers = [int(i) for i in string_with_game[string_with_game.find('|') + 1 :].split()]
        temp_list.append(ticket_numbers)
        temp_list.append(user_numbers)
        mapped_game[num_of_game] = temp_list
